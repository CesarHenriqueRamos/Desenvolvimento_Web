<?php
  ...
  if(isset($_POST['atualisa'])){
    ...
    $sobre = $_POST['sobre'];
    $id = 1;
    $atualisa = $pdo->prepare('UPDATE sobre SET sobre = ? WHERE id = ?');
    if($atualisa->execute(array($sobre,$id))){
      $sobre = $pdo->prepare("SELECT * FROM sobre");
      $sobre->execute();
      $sobre = $sobre->fetch();
    }
                            ...
                            <?php
                              if(isset($_POST['acao'])){
                                ...
                                if($nome == ''){
                                  ...
                                  $inserir = $pdo->prepare("INSERT INTO equipe VALUES(null,?,?)");
                                  $inserir->execute(array($nome,$descricao));
